...
exports.run = async (client, msg, args) => {
  ...
  if (!msg.member.hasPermission("MANAGE_ROLES")) {
    ...
    let index = client.applications.get(msg.guild.id, "applications").findIndex(obj => obj.author === user.id)
    if (index < 0) return msg.reply('This user does not have any active applications!')
    ...
    .setDescription(paginated.items.map(i => `__**${i.question}**__\n${i.answer}`))
    .setColor("RANDOM")
   ...
   await m.react("❌")
			...
			const filter = (reaction, user) => reaction.emoji.name === "◀️" && user.id === msg.author.id || reaction.emoji.name === "▶️" && user.id === msg.author.id || reaction.emoji.name === "❌" && user.id === msg.author.id 
   ...
   let editEmbed = new MessageEmbed({
   	...
   	description: paginatedAfter.items.map(i => `__**${i.question}**__\n${i.answer}`).join(" "),
   	footer: {
          text: `Page ${i} out of ${client.applications.get(msg.guild.id, "application").length}`,
   	}
   })
      m.edit(editEmbed)
    ...
    .setColor("RANDOM")
    .setDescription(paginated.items.map(i => `***${client.users.cache.get(i.author) ? client.users.cache.get(i.author).tag : "Unknown User"}(${i.author})*** => ${moment(i.registered).format("LLLL")}`))
